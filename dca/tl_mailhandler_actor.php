...
class tl_mailhandler_actor extends Backend
{
    ...
    public function loadActorConfig(DataContainer $dc)
    {
        ...
        if (array_key_exists($dc->getData("provider"), $GLOBALS['TL_MAILHANDLER_ACTORS']))
        {
            ...
            $GLOBALS['TL_DCA']['tl_mailhandler_actor']['palettes']['default'] .= ';{areaHandleConfig},'.implode(",",array_keys($arrDCA));
            
            $GLOBALS['TL_DCA']['tl_mailhandler_actor']['fields'] = array_merge($GLOBALS['TL_DCA']['tl_mailhandler_actor']['fields'],$arrDCA);
